...
function searchByRival(rivalSubstring) {
  let matching_games = gamesData.filter((game, index) =>
    game.rival.toLowerCase().includes(rivalSubstring.toLowerCase())
  )
  // Return matching games and their index in the original data using indexOf
  return matching_games.map((game, index) => ({
    index: gamesData.indexOf(game),
    game,
  }))
}
